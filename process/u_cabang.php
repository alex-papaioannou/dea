<?php
	...
	if ((ISSET($_GET['id'])) AND (ISSET($_POST['cabang_klinik'])) AND (ISSET($_POST['alamat']))) {
		...
		$query = 'SELECT COUNT(cabang_klinik) AS total FROM klinik WHERE LOWER(cabang_klinik)=LOWER("'.$cabang.'") AND LOWER(alamat)=LOWER("'.$alamat.'")';
		if (mysqli_query($conn, $query)) {
			...
			if ($data['total'] > 0) {
				...
				$query = 'SELECT COUNT(*) AS total FROM klinik WHERE LOWER(cabang_klinik)=LOWER("'.$cabang.'") AND LOWER(alamat)=LOWER("'.$alamat.'") AND id_klinik="'.$id.'"';
				$query = mysqli_query($conn, $query);
				...
				$query3 = 'SELECT COUNT(*) AS total FROM klinik WHERE LOWER(cabang_klinik)=LOWER("'.$cabang.'") OR LOWER(alamat)=LOWER("'.$alamat.'")';
				if (mysqli_query($conn, $query3)) {
					...
					if ($data3['total'] > 0) {
						...
						$q = mysqli_query($conn, 'SELECT COUNT(*) AS total FROM klinik WHERE LOWER(cabang_klinik)=LOWER("'.$cabang.'") AND LOWER(alamat)=LOWER("'.$alamat.'") AND id_klinik="'.$id.'"');
						$d = mysqli_fetch_assoc($q);
						if ($d['total'] == 0) {
							// Data disimpan
							$query = 'UPDATE klinik SET cabang_klinik="'.$cabang.'", alamat="'.$alamat.'" WHERE id_klinik="'.$id.'"';
							if (mysqli_query($conn, $query)) {
