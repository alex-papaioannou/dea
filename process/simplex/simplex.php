<?php 
	...
	if($_GET['loop'] == 0){
		mysqli_query($conn, "DELETE FROM tb_perhitungan_efisiensi");
	}
	...
	$dmu = $_GET['dmu'];
	//mendapatkan data z -> jumlah semua paramter input untuk dmu x
	$qdmu = mysql_query("SELECT * FROM tb_detail_dmu d, tb_kriteria k WHERE k.id_kriteria = d.id_kriteria AND d.id_dmu='$dmu' AND k.indikator='0' ORDER BY k.id_kriteria");
	$z = 0;
	while($f = mysql_fetch_array($qdmu)){
		$z += $f['nilai_dmu'];
	}
	
	//mendapatkan semua data variabel input.
	$sql = mysql_query("SELECT * FROM tb_detail_dmu d, tb_kriteria k WHERE k.id_kriteria = d.id_kriteria AND d.id_user='$id_user' AND indikator='0' ORDER BY k.id_kriteria ASC, d.id_dmu ASC");
	$pers = array();
	$i=-1;
	$cur = 0;
	
	//membuat array yang isinya semua parameter input 
	while($f = mysql_fetch_array($sql)){
		if($cur != $f['id_kriteria']){
			$subpers = array();
			$i++;
			$cur = $f['id_kriteria'];
			$pers[$i][] = ($f['nilai_dmu']);
	...
	//mendapatkan jumlah dari setiap dmu
	$input = array();
	for($i=0;$i<count($pers);$i++){
		for($j=0;$j<count($pers[$i]);$j++){
			if(!ISSET($input[$j])){ $input[$j] = 0;}
			$input[$j] = $input[$j] + $pers[$i][$j];
		}
	}
	
	//membuat persamaan
	$final_input = ($input[0]/$z).'x1';
	for($i=1;$i<count($input);$i++){
		$final_input .= '+'.($input[$i]/$z).'x'.($i+1);
	}
	...
	/*============================================================Output================================================================================*/
	//mendapatkan data z -> jumlah semua paramter output untuk dmu x
	$qdmu = mysql_query("SELECT * FROM tb_detail_dmu d, tb_kriteria k WHERE k.id_kriteria = d.id_kriteria AND d.id_dmu='$dmu' AND k.indikator='1' ORDER BY k.id_kriteria");
	$z = array();
	while($f = mysql_fetch_array($qdmu)){
		$z[] = $f['nilai_dmu'];
	}
	
	//mendapatkan semua data variabel output.
	$sql = mysql_query("SELECT * FROM tb_detail_dmu d, tb_kriteria k WHERE k.id_kriteria = d.id_kriteria AND d.id_user='$id_user' AND indikator='1' ORDER BY k.id_kriteria ASC, d.id_dmu ASC");
	$pers = array();
	$i=-1;
	$cur = 0;
	
	//membuat array yang isinya semua parameter output 
	while($f = mysql_fetch_array($sql)){
		if($cur != $f['id_kriteria']){
...
<script language="Javascript" src="simplex_files/authenticationscripts.js">
</script>
<link rel="stylesheet" href="simplex_files/utilsStyles.css" type="text/css">
<script language="Javascript" src="simplex_files/jquery.js"></script>
...
if  (TableauNumber > maxSteps) document.theSpreadsheet.expr.value = "No optimal solution found after 50 steps. Aborted."
	...
	document.theSpreadsheet.expr.value = "" + objectiveName + " = ";
...
$(document).ready(function(){
	doIt(2); 
	doIt(1);
	var inputHasil = $('.expr').val();
	$('.tes').html(inputHasil);
	//alert(inputHasil);
	$.ajax({
		type : 'POST',
		data : 'hasil='+inputHasil+'&id_dmu=<?php echo $dmu;?>',
		url  : 'hasil.php',
...
<script src="simplex_files/fpc.js" type="text/javascript" defer="defer"></script>
<body alink="red" bgcolor="EEFFF7" link="purple" text="001111" vlink="purple" style='display:none;'>
...
<textarea name="input" rows="10" cols="80">Minimize z = <?php echo $final_input; ?> subject to
<?php 
	$data = "";
...
<tr><td align="center">Solution:<br><input size="80" name="expr" class='expr' value="" type="text">

<br>

<input value="Solve" onclick="doIt(2); doIt(1)" type="button"> &nbsp;

<input value="Example" onclick="doIt(5)" type="button"> &nbsp;

<input value="Erase Everything" onclick="doIt(3)" type="button"> &nbsp;

Rounding: <input size="2" value="6" name="acc" type="text"> significant digits &nbsp;

<br>Mode: <select name="Mode" size="3"> 

	<option selected="selected">Decimal
...
<?php 
	$sqlaa = mysql_query("SELECT * FROM tb_dmu WHERE id_user='$id_user' ORDER BY id_dmu ASC LIMIT ".($_GET['loop']+1).",1");
	if(mysql_num_rows($sqlaa) > 0){
	$fff = mysql_fetch_array($sqlaa);
?>
<meta http-equiv="refresh" content="0; url=simplex.php?dmu=<?php echo $fff['id_dmu']; ?>&loop=<?php echo ($_GET['loop']+1)?>">
<?php }else{ ?>
<meta http-equiv="refresh" content="0; url=../main.php?h=perhitungan_akhir">
<?php } ?>